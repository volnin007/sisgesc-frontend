...
export default function DeclaracaoTransferenciaPage() {
  ...
  async function emitir(e: FormEvent) {
    ...
    try {
      ...
      setEmitido(data);
      setMsg('Documento emitido. Use Imprimir / PDF.');
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      <div className="flex flex-wrap justify-between gap-3 no-print">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">Modelos SEDUC-GO adaptados · Dimas Nasser</p>
          </div>
        </div>
      ...
      <form onSubmit={emitir} className="bg-white rounded-2xl border p-5 space-y-3 shadow-sm no-print">
        <div className="flex gap-4 text-sm font-semibold">
          <label className="flex items-center gap-2">
            <input type="radio" checked={tipoDocumento === 'DECLARACAO'} onChange={() => setTipoDocumento('DECLARACAO')} /> Declaração
          </label>
          <label className="flex items-center gap-2">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <label className="text-sm font-semibold">Detalhe da situação
          ...
          <label className="text-sm font-semibold">% Frequência
            <input type="number" value={percentualFrequencia} onChange={(e) => setPercentualFrequencia(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2.5" />
          </label>
          ...
          <label className="text-sm font-semibold">Estado destino
            <input value={estadoDestino} onChange={(e) => setEstadoDestino(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2.5" />
          </label>
        </div>
      ...
      {/* Impressão */}
      <div className="bg-white border rounded-2xl p-8 text-slate-900 text-sm space-y-4 print:border-0">
        <div className="text-center space-y-1 border-b pb-3">
          <p className="text-xs font-bold uppercase tracking-wide">Estado de Goiás · Secretaria de Estado da Educação – SEDUC/GO</p>
          <p className="font-bold">Escola Municipal Dimas Nasser</p>
          <p className="text-xs">Município: Bom Jardim de Goiás · Código INEP: (conforme cadastro)</p>
        ...
        {tipoDocumento === 'DECLARACAO' ? (
          <div className="space-y-3 leading-relaxed">
            <p>
              Declaramos, para os devidos fins, que o(a) estudante <strong>{aluno?.nomeCompleto || '________________'}</strong>,
              inscrito(a) no CPF nº <strong>{aluno?.cpf || '________________'}</strong>, nascido(a) em{' '}
              <strong>{aluno?.dataNascimento ? new Date(aluno.dataNascimento).toLocaleDateString('pt-BR') : '__/__/____'}</strong>,
              filho(a) de <strong>{aluno?.nomeMae || '________________'}</strong>,
              esteve regularmente matriculado(a) nesta unidade escolar no Ano Letivo de{' '}
              <strong>{new Date().getFullYear()}</strong>, cursando o(a) <strong>{matricula?.turma?.anoSerie || serieLabel(matricula)}</strong>,
              no turno <strong>{matricula?.turma?.turno || '________'}</strong>.
            </p>
            <p>
              Informamos que, por solicitação do(a) responsável legal, foi concedida a <strong>Transferência Escolar</strong>,
              ficando o(a) estudante apto(a) à continuidade de seus estudos em outra unidade de ensino, conforme a legislação vigente.
            </p>
            <p>
              O <strong>Histórico Escolar</strong> será entregue juntamente com esta declaração ou encaminhado à unidade de destino,
              observados os prazos da SEDUC-GO.
            </p>
            <p>
              A presente declaração tem validade de <strong>30 (trinta) dias</strong>, contados da data de emissão.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <p><strong>Nome:</strong> {aluno?.nomeCompleto || '—'} · <strong>Nasc.:</strong> {aluno?.dataNascimento ? new Date(aluno.dataNascimento).toLocaleDateString('pt-BR') : '—'} · <strong>CPF:</strong> {aluno?.cpf || '—'}</p>
            <p><strong>Mãe:</strong> {aluno?.nomeMae || '—'} · <strong>Matrícula:</strong> {matricula?.numeroMatricula || '—'}</p>
            <p><strong>Etapa/série:</strong> {matricula?.turma?.nome || '—'} · <strong>Turno:</strong> {matricula?.turma?.turno || '—'}</p>
            <p><strong>Situação:</strong> {motivo}{situacao ? ` — ${situacao}` : ''}</p>
            <p><strong>Frequência:</strong> {percentualFrequencia ? `${percentualFrequencia}%` : '—'} · <strong>Último dia:</strong> {dataUltimaFrequencia ? new Date(dataUltimaFrequencia + 'T12:00:00').toLocaleDateString('pt-BR') : '—'}</p>
            <p><strong>Documentos entregues:</strong> {docs.join(', ') || '—'}</p>
            <p><strong>Destino:</strong> {escolaDestino || '—'} — {municipioDestino || '—'}/{estadoDestino || '—'}</p>
            {observacoes && <p><strong>Observações:</strong> {observacoes}</p>}
            <p className="leading-relaxed">
              Declaramos que o(a) estudante esteve regularmente matriculado(a) nesta unidade, apto(a) à continuidade dos estudos na unidade de destino, conforme normas da SEDUC-GO.
            </p>
          </div>
        )}

        <p className="pt-4">Município: Bom Jardim de Goiás · Data: {new Date().toLocaleDateString('pt-BR')}</p>

        <div className="grid sm:grid-cols-2 gap-10 pt-12 text-center text-xs">
          <div>
            <div className="border-t border-slate-800 pt-1 mx-6">Secretário(a) Escolar</div>
            <p className="mt-1">{secretariaNome || 'Nome: _______________'}{secretariaMatricula ? ` · Matrícula: ${secretariaMatricula}` : ''}</p>
          </div>
          <div>
            <div className="border-t border-slate-800 pt-1 mx-6">Diretor(a)</div>
            <p className="mt-1">{diretorNome || 'Nome: _______________'}{diretorMatricula ? ` · Matrícula: ${diretorMatricula}` : ''}</p>
          </div>
        </div>
        <p className="text-[10px] text-center text-slate-500 pt-6">Carimbo oficial da unidade escolar · SISGESC · Volnin Tech Hacker</p>
      </div>

      {lista.length > 0 && (
        <p className="no-print text-xs text-slate-500">{lista.length} documento(s) emitido(s) · último: #{emitido?.id || lista[0]?.id}</p>
      )}
    </div>
  );
}

function serieLabel(matricula: any) {
  return matricula?.turma?.anoSerie || matricula?.turma?.nome || '________';
}
