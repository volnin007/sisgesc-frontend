  ...
  return (
    <aside className="w-64 bg-[#0a0f1a] text-white min-h-screen p-4 flex flex-col border-r border-cyan-500/10 shrink-0 hidden md:flex">
      <div className="mb-6 flex flex-col items-center">
        <h1 className="text-lg font-black tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-green-400">
          VOLNIN
        </h1>
      ...
      <nav className="flex-1 space-y-1">
        {menu.map((m) => {
          const active = pathname === m.href || (m.href !== '/' && pathname.startsWith(m.href.split('/')[1] ? `/${m.href.split('/')[1]}` : m.href));
          const isActive =
            m.href === '/'
              ? pathname === '/'
              : pathname === m.href || pathname.startsWith(m.href.replace(/\/\d+$/, ''));
          return (
            <Link
              key={m.href}
              href={m.href}
      ...
      <div className="mt-6 pt-4 border-t border-white/10 space-y-3">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-[9px] text-gray-500">Gestão 2025/2028</p>
        </div>
      </div>
    </aside>
  );
